...
+ (NSString *)diskSpace {
	...
	@try {
        ...
        NSString *diskSpac = [self formatMemory:space];
        
        // Check to make sure it's valid
        if (diskSpac == nil || diskSpac.length <= 0) {
...
+ (NSString *)freeDiskSpace:(BOOL)inPercent {
	...
	@try {
        ...
        if (inPercent) {
            ...
            diskSpac = [NSString stringWithFormat:@"%.f%%", PercentDiskSpace];
        } else {
            // Turn that long long into a string
            diskSpac = [self formatMemory:Space];
        }
...
+ (NSString *)usedDiskSpace:(BOOL)inPercent {
    ...
    @try {
        ...
        if (inPercent) {
            ...
            // Convert that float to a string
            usedDiskSpac = [NSString stringWithFormat:@"%.f%%", percentUsedDiskSpace];
        } else {
            // Turn that long long into a string
            usedDiskSpac = [self formatMemory:uds];
        }
        
        // Check to make sure it's valid
        if (usedDiskSpac == nil || usedDiskSpac.length <= 0) {
            // Error, diskspace not given
            return nil;
        }
...
+ (long long)longDiskSpace {
	...
	@try {
        ...
        if (error == nil) {
            ...
            diskSpac = [[fileAttributes objectForKey:NSFileSystemSize] longLongValue];
        } else {
            // Error, return nil
            return -1;
        }
